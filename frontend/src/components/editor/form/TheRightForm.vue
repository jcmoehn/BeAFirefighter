...
<template>
  <div>
    <b-card>
      <b-card-header>
        <b-row>
        <b-col><h4>Infos über die rechte Seite deines Autos</h4></b-col>
      ...
      <b-card>
        ...
        <!-- Editor zum Hochladen des Bildes der rechten Außenseite -->
        <b-form-group
          label="Bitte lade hier die rechte Bild-Seite deines Autos hoch :"
        ...
        <div v-if="zoom">
          <b-form v-for="(i, indexZoom) in rechts.hitBoxen" :key="indexZoom">
            <hr class="hit">
            <b-form-group
              :label="`Bitte lade hier die rechte Nah-Ansicht deiner ${indexZoom+1}. HitBox hoch:`"
            ...
            >
              ...
              <div>
                <b-form v-for="(i,indexTool) in rechts.zoomBoxen[indexZoom]" :key="indexTool">
                  <hr class="zoom">
                  <b-form-group
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        hitBoxen(coords) {
          this.rechts.hitBoxen = coords
          this.zoom = true;
        },
        getFrontImage(image){
          this.rechts.bild = image;
        },
        zoomBoxen(coords) {
          this.rechts.zoomBoxen.push(coords)
          this.tool.push([])
          this.extraTool.push([])
        },
        getZoomImage(image) {
          this.rechts.zoomImg.push(image)
        },
        saveAll() {
          this.rechts.werkzeuge = this.tool;
          this.rechts.sonderwerkzeuge = this.extraTool;
          console.log("TheRightForm.saveAll(): " + this.rechts);
          console.log(this.rechts);
          this.$emit('rechts', this.rechts);
        },
