...
<template>
  <div>
    <b-card>
      <b-card-header>
        <b-row>
        <b-col><h4>Infos über die obere Seite deines Autos</h4></b-col>
      ...
      <b-card>
        ...
        <!-- Editor zum Hochladen des Bildes der oberen Außenseite -->
        <b-form-group
          label="Bitte lade hier die obere Bild-Seite deines Autos hoch :"
        ...
        <div v-if="zoom">
          <b-form v-for="(i, indexZoom) in oben.hitBoxen" :key="indexZoom">
            <hr class="hit">
            <b-form-group
              :label="`Bitte lade hier die obere Nah-Ansicht deiner ${indexZoom+1}. HitBox hoch:`"
            ...
            >
              ...
              <div>
                <b-form v-for="(i,indexTool) in oben.zoomBoxen[indexZoom]" :key="indexTool">
                  <hr class="zoom">
                  <b-form-group
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        hitBoxen(coords) {
          this.oben.hitBoxen = coords
          this.zoom = true;
        },
        getFrontImage(image){
          this.oben.bild = image;
        },
        zoomBoxen(coords) {
          this.oben.zoomBoxen.push(coords)
          this.tool.push([])
          this.extraTool.push([])
        },
        getZoomImage(image) {
          this.oben.zoomImg.push(image)
        },
        saveAll() {
          this.oben.werkzeuge = this.tool;
          this.oben.sonderwerkzeuge = this.extraTool;
          console.log("TheTopForm.saveAll(): " + this.oben);
          console.log(this.oben);
          this.$emit('oben', this.oben);
        },
